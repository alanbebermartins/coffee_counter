...
class Drink {
    ...
    public function historyByUser($userId) {
        $stmt = $this->pdo->prepare("SELECT DATE(created_at) as date, SUM(quantity) as times 
                                     FROM drinks 
                                     WHERE user_id = :uid 
    ...
    public function rankingByDate($date) {
        $stmt = $this->pdo->prepare("SELECT u.name, SUM(d.quantity) as times 
                                     FROM drinks d 
    ...
    // ranking last X days (corrigido)
    public function rankingLastDays($days) {
        $days = (int)$days;
        $sql = "SELECT u.name, SUM(d.quantity) as times 
                FROM drinks d 
                JOIN users u ON u.id = d.user_id 
